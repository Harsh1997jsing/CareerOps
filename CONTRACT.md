...
| POST | `/explore/save` | bearer | `ExploreSaveRequest` | `ExploreSaveResponseOut` | **Shared save route** for every discovery source, not Explore-only — see route docstring. The only way a job from Explore, `/targets/search`, `/scrape/jobspy`, or AI Search enters `jobs`. A newly inserted (non-duplicate) job is also queued for automatic hard-filter/score analysis as a background task — the response doesn't wait for it, so `inserted: true` doesn't mean analysis has finished yet; poll `GET /jobs/{id}` or the Job Detail page to see the result land |
